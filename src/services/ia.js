...
export async function ChatMensagem(pergunta) {
  try {
    ...
    const data = `${JSON.stringify(pergunta)} - RESPONDA SEMPRE MINHA ÚLTIMA PERGUNTA, PORÉM LEVE EM CONTA TODAS AS OUTRAS PERGUNTAS E RESPOSTA QUE EXISTEM NA LISTA. VOCÊ DEVE SER UM AGENTE DE IA CONFORME A PRIMEIRA PERGUNTA DE CADA LISTA QUE RESPONDER, NÃO PODE SAIR DO SEU TEMA, APENAS PODE RESPONDER TODAS AS PERGUNTAS SE FOR O ASSINTENTE GERAL. É EXTREMAMENTE PROIBIO VOCÊ DAR A RESPOSTA PARA O ALUNO, VOCÊ DEVE EXPLICAR COM O ALUNOS CHEGA NO SEU OBJETIVO, SEJA CONTEX.`;
